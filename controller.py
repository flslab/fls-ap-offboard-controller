...
class Controller:
    ...
    def connect(self):
        if self.mavproxy:
            ...
            cmd = [
                ...
                "--load-module=vicon",
                f"--cmd=\"vicon set; vicon set object_name {args.obj_name}; vicon set save_init_pos {local_coords_path}; vicon set save_pos_log {vicon_pos_log_path}; vicon set vicon_stop_trigger {vicon_stop_trigger}; vicon set vision_rate 100; vicon set gps_rate 10; vicon set; vicon start;\"",
                "--daemon",
            ]
    ...
    def test_trajectory_3(self):
        waypoints = [
                        [0, -.8, -self.takeoff_altitude],
                        [0, .8, -self.takeoff_altitude],
                    ] * 5

        waypoints_2 = [
                          [-.8, 0, -self.takeoff_altitude],
                          [.8, 0, -self.takeoff_altitude],
                      ] * 5
